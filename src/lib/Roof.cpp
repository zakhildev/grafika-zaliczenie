...
Roof::Roof() {
  ...
  diffuse.path = "textures/roof_diffuse.jpg";
  ...
  if (tex_diffuse) {
    ...
    std::cerr << "Failed to load Roof diffuse texture." << std::endl;
  }

  Texture specular;
  specular.type = "texture_specular";
  specular.path = "textures/roof_specular.jpg";
  ...
  if (tex_specular) {
    ...
    std::cerr << "Failed to load Roof specular texture." << std::endl;
  }

  mesh = new Mesh(vertices, indices, textures);
}

Roof::~Roof() { delete mesh; }

void Roof::Draw(Shader &shader) { mesh->Draw(shader); }
