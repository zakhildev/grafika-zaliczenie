...
void main() {
  ...
  // Light 1
  vec3 lightDir1 = normalize(lightPos1 - viewPos);
  float diff1 = max(dot(norm, lightDir1), 0.0);
  vec3 diffuse1 = diff1 * lightColor1;

  // Light 2
  vec3 lightDir2 = normalize(lightPos2 - viewPos);
  ...
  float spec1 = pow(max(dot(norm, halfwayDir1), 0.0), 45.0);
  ...
  float spec2 = pow(max(dot(norm, halfwayDir2), 0.0), 45.0);
